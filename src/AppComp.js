...
class AppComp extends Component {
  ...
  render() {
    ...
    const listItems = items.map(item => (
      <li key={item.id} onClick={() => this.onSelect(item.id)}>
        <span className="item">{item.id}</span> | {item.first_name} |
        {item.last_name}
      </li>
    ));
    return (
      <div className="App">
        ...
        <br />
        <button type="button" name="button" onClick={() => this.onNewItem()}>
          New Item
        </button>
        <br />
        {newItem && (
          <NewItem onSubmit={this.onCreateItem} onCancel={this.onCancel} />
        )}
  ...
  onSelect(id) {
    ...
    console.log("end of on select method...");
  }
  onCancel() {
    this.clearState();
  }
